...
@Composable
fun AccountRequestsScreen(
    onBack: () -> Unit,
    viewModel: AccountRequestsViewModel = hiltViewModel()
) {
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect {
            if (it is AccountRequestsEvent.Toast) snackbar.showSnackbar(it.message)
        }
    }

    BankaScaffold(
        title = "Zahtevi za racune",
        onBack = onBack,
                ...
                GlassCard(modifier = Modifier.fillMaxWidth()) {
                    Row(verticalAlignment = Alignment.CenterVertically) {
                        Column(modifier = Modifier.weight(1f)) {
                            Text(req.clientName ?: req.clientEmail ?: "Klijent", style = MaterialTheme.typography.titleSmall, color = MaterialTheme.colorScheme.onSurface)
                            Text(
                            ...
                            req.initialDeposit?.let {
                                Text("Pocetni depozit: ${MoneyFormatter.format(it, 2)}", style = MaterialTheme.typography.labelSmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                            }
                        }
    ...
    rejectingId?.let { id ->
        ...
        AlertDialog(
            ...
            text = { AppTextField(value = reason, onValueChange = { reason = it }, label = "Razlog", singleLine = false, modifier = Modifier.fillMaxWidth()) },
            confirmButton = {
                TextButton(onClick = {
