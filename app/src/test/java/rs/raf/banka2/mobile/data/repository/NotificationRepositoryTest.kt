 ...
 * - filter ALL vraca read=null query param
 * - filter UNREAD vraca read=false query param
...
class NotificationRepositoryTest {
    ...
    @Test
    fun `list ALL passes read=null query param`() = runTest {
        val page = NotificationPageDto(
            content = listOf(
                NotificationDto(
                    id = 1, type = "PAYMENT_RECEIVED", title = "T", message = "M",
                    read = false, createdAt = "2026-05-26T10:00:00Z"
                )
            ),
            totalElements = 1, totalPages = 1, number = 0, size = 20
        )
        coEvery { api.list(read = null, page = 0, size = 20) } returns Response.success(page)
        ...
        coVerify { api.list(read = null, page = 0, size = 20) }
    }

    @Test
    fun `list UNREAD passes read=false query param`() = runTest {
        val page = NotificationPageDto(content = emptyList(), totalPages = 0)
        coEvery { api.list(read = false, page = 0, size = 20) } returns Response.success(page)

        repo.list(NotificationFilter.UNREAD)

        coVerify { api.list(read = false, page = 0, size = 20) }
    }
